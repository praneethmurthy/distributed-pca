...
signal_energy_range = [1.1, 2.2, 3.3];
...
conv_norm_mat = zeros(power_iter + 1, length(signal_energy_range), MC);

conv_batch_mat = zeros(power_iter + 1, length(signal_energy_range), MC);


for mc = 1 : MC
    fprintf('%d mc iteration', mc);
%% generate the "rectangular" data points, and also the sample covariance
ctr =1;
for signal_energy = signal_energy_range
...
l1 = legend('tb = 1, R = 1.1', 'tb = 1, R = 2.2', 'tb = 1, R = 3.3', 'tb = 10, R=1.1', 'tb = 10, R=2.2', 'tb = 10, R=3.3');
l1.FontSize = 15;
