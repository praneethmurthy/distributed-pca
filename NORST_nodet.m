function [FG,BG, t_hat, P_track_full] = NORST_nodet(M,T_obs,P_init,ev_thresh,alpha,K,omega,tol)
...
opts.delta=0.4;
opts.print = 0;
fprintf('The eigenvalue threshold is %.3f\n', sqrt(alpha * ev_thresh)); 
%% Main Algorithm Loop
for ii = 1 : t_max
    %% Estimate support
...
%     T_hat(t_hat_temp, ii) = 255;
    ...
    if(~mod(ii + 1 , alpha))
        ...
        %MM = phi.times(L_temp);        
%         if(~ph)     %%detect phase            
%             fprintf('the singular value at t = %d is %.3f\n', ii, svds(MM,1));
...
%simpleEVD_fed(L_hat(:, idx), r, 500, 1, 1e-6)
%             [P_hat,~] = simpleEVD_fed((L_hat(:, max(1, ii - alpha + 1) : ii)), r, 500, 1, 1e-6);
            P_hat = simpleEVD_fed((L_hat(:, max(1, ii - alpha + 1) : ii)), r, 500, 1, 1e-6);
